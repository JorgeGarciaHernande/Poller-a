...
class CarritoController extends ChangeNotifier {
  ...
  void agregarProductoAlCarrito({
    ...
    if (index != -1) {
      ...
      _productosSeleccionados.add({
        ...
        'nota': '', // Campo para la nota inicializado vacío
      });
    }
  ...
  // Agregar una nota a un producto específico en el carrito
  void agregarNotaProducto(int index, String nota) {
    if (index >= 0 && index < _productosSeleccionados.length) {
      _productosSeleccionados[index]['nota'] = nota;
      notifyListeners(); // Notificar a la vista para que se actualice
      print("Nota agregada al producto: ${_productosSeleccionados[index]['nombreProducto']}");
    }
  }

  // Obtener una nota general concatenada de todos los productos
  String obtenerNotasGenerales() {
    return _productosSeleccionados.map((producto) {
      final nombre = producto['nombreProducto'];
      final nota = producto['nota'] ?? '';
      return '$nombre: $nota';
    }).join('; '); // Concatenar cada nota con un separador
  }
}
