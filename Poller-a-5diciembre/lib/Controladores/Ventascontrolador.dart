...
class VentaController {
  ...
  Future<int> obtenerUltimoIdVenta() async {
    try {
      QuerySnapshot snapshot = await ventas.orderBy('fecha', descending: true).limit(1).get();
      if (snapshot.docs.isNotEmpty) {
        final ultimoId = int.parse(snapshot.docs.first.id);
        return ultimoId;
      } else {
        return 0; // Si no hay ventas, el primer ID será 1
      }
    } catch (e) {
  ...
  Future<int> obtenerIdTicketMasNuevo() async {
    try {
      QuerySnapshot snapshot = await ventas.orderBy('fecha', descending: true).limit(1).get();
      if (snapshot.docs.isNotEmpty) {
